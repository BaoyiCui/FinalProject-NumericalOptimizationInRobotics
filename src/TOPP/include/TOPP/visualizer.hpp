...
private:
    ros::NodeHandle nh;
    ros::Publisher routePub;     // publisher for path
    ros::Publisher wayPointsPub; // publisher for way points
...
public:
    ...
    // Visualize some polytopes in V-representation
    inline void visualizePolytope(const std::vector<Eigen::Matrix3Xd> &vPolys)
    {
        ...
        {
            oldTris = mesh;
            Eigen::Matrix<double, 3, -1, Eigen::ColMajor> vPoly = vPolys[id];
